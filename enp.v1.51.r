...
enp.FUN <- function( data, 
  ...
  if(!seats %in% colnames(data)){
    ...
    data <- as_tibble(data.frame(data, NA))
  ...
  if(enp_v){
    ...
    data.enp <- as_tibble(merge( data, data.store, by = c("year", "chamber", "district")))
  ...
  if(enp_v & enp_s){
    ...
    ### Merging Data
    enp.data <- as_tibble(merge(enpv.store, enps.store, by = c("year", "chamber", "district"), all = T))
